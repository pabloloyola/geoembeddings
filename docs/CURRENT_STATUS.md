...
| R12 | partial | `audit-privacy` | T4.3a provides authenticated diagnostic-control inputs; frozen membership and sensitive-attribute attacks and user-level splits; confidence intervals; explicit support, coverage, exclusions, and applicability; and separate utility/privacy axes. Membership can be unavailable without authenticated same-lineage members and non-members, no `selected_candidate` exists, and T4.3 remains incomplete. This simulator audit is neither a privacy guarantee nor deployment certification. |
