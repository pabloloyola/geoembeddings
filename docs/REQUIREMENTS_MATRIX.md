...
|---|---|---|---|---|---|
| R10 | Representation uncertainty | Low-evidence or unstable users receive lower confidence | window/event bootstrap or stochastic views | embedding variance, reliability-error relation, coverage-risk | Partial/executable: T1.7 provides cutoff-bootstrap repeatability diagnostics over frozen cutoffs, distinct from the completed T4.1a held-out-user window-bootstrap calibration. `calibrate-reliability` produces raw and calibrated reliability-error bins and coverage-risk curves for authenticated `diagnostic_control` representations. No immutable `selected_candidate` exists, so selection-dependent T4.1 calibration evidence remains pending; the diagnostic-control result does not establish deployment calibration. |
| R11 | Nonstationarity | Temporary change decays; sustained change updates persistent state | matched transient and sustained-change simulations | adaptation/forgetting time, permanent drift, regret over time | Partial/executable: the T4.2 diagnostic-control adaptation/forgetting audit is executable and complete, with authenticated no-change, temporary, and sustained inputs plus separate coverage and uncertainty. No representation has the `selected_candidate` role, so the selection-dependent R11 conclusion remains unavailable; simulator-defined change semantics are not deployment evidence |
| R12 | Privacy | Embedding does not unnecessarily memorize identities or sensitive attributes | membership/attribute audit splits | attack AUC, sensitive probe performance, utility/privacy curve | Partial/executable: T4.3a provides an authenticated diagnostic-control audit with frozen attacks/splits, explicit applicability and support, confidence intervals, coverage/exclusions, and separate utility/privacy axes. Membership remains unavailable when authenticated same-lineage member/non-member participation is absent. No representation has the `selected_candidate` role, so the selected-candidate R12 conclusion remains unavailable; simulator attacks provide neither a formal privacy guarantee nor deployment certification, and AUC near 0.5 would not prove safety. |
| R13 | Computational efficiency | Feasible offline training and online update/export | benchmark harness | time, peak memory, examples/s, update latency, artifact size | Partial/executable: T1.7 CPU offline measurements and the implemented T4.4 online benchmark harness cover immutable cold-start, single-event, frozen-batch, and export workloads for baseline and learned diagnostic controls, with full-recomputation correctness checks. Training efficiency and comparable hardware/reference-scale evidence remain unavailable; the hardware-specific synthetic measurements do not select a representation or establish deployment feasibility |
