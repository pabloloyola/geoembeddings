...
|---|---|---|---|
| **T1.16 — Observed-only embedding visualization** | R1--R4, R8 diagnostic scope only | `uv run pytest tests/test_embedding_visualization.py`; `uv run --extra viz geoembed visualize-embeddings --experiment-dir EXPERIMENT_DIR --kind learned --reference-cutoff train` | `visualization/{learned,baseline}{_dense}/`; versioned reducer metadata, row-aligned projections, small multiples, and trajectories. No new scientific baseline artifact: the command visualizes an existing authenticated export and does not establish semantics, disentanglement, invariance, or recommendation quality. Affected layer is observed-only diagnostic tooling/documentation; simulator, observed contract, model, and protected evaluator are unchanged. |
